...
class ReportByYearDataTable extends BlankDataTable
{
	public function __construct()
	{
		$this->routeResource = 'report_by_years';
	}
	/**
	...
	{
		$result = '';
		if(auth()->user()->hasPermissions($this->routeResource.".getByMonth")) {
			$result .= '<a href="'.route($this->routeResource.".getByMonth", $row->year).'">
				<button type="button" class="btn btn-default" title='.__('show').'>
					<i class="bx bxs-calendar text-primary actions_icon"></i>
				</button>
			</a>';
		}
		if(auth()->user()->hasPermissions($this->routeResource.".getYearDetail")) {
			$result .= '<a href="'.route($this->routeResource.".getYearDetail", $row->year).'">
	...
	public function dataTable($query)
	{
		...
		$datatable->filter(function($query) {
			if(request('initial_date') !== null && request('final_date') !== null){
				$query->where(DB::raw('YEAR(selling_rows.created_at)'),'>=' ,request('initial_date'))
					->where(DB::raw('YEAR(selling_rows.created_at)'),'<=' ,request('final_date')." 23:59:59");
			}
	...
	{	
		...
		return [
			...
			'columnDefs' => [
				...
				['width' => '20%', 'targets' => 1, 'className' => 'text-end'],
			...
			'footerCallback' => 'function() { footerCustomize([1,2,3], this.api(), [0,1,2]) }'
