...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('product_values', function (Blueprint $table) {
			$table->increments('id')->comment('Identificador de la tabla de product_values');

            //Llaves foráneas
            $table->integer('product_id')->unsigned()->comment('Llave foránea de products');
            $table->foreign('product_id')->references('id')->on('products');
            $table->integer('product_attribute_id')->unsigned()->comment('Llave foránea de product_attributes');
            $table->foreign('product_attribute_id')->references('id')->on('product_attributes');

            //
            $table->string('value')->comment('Valor dinámico');
