...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('buying_rows', function (Blueprint $table) {
			$table->increments('id')->comment('Identificador de la tabla de buying_rows');
            ...
            //
            $table->string('description')->comment('Descripción del producto comprado');
            $table->string('amount')->comment('Cantidad del producto comprado');
            $table->float('unit_cost', 12, 4)->comment('Costo unitario del producto');
            $table->float('iva')->comment('Porcentaje del IVA');
            $table->float('total_cost', 12, 4)->comment('Costo total del producto');
