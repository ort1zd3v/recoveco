...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('buyings', function (Blueprint $table) {
			$table->increments('id')->comment('Identificador de la tabla de buyings');

            //Llaves foráneas
            $table->integer('supplier_id')->unsigned()->comment('Llave foránea de suppliers');
            $table->foreign('supplier_id')->references('id')->on('suppliers');

            //
            $table->float('subtotal', 12, 4)->comment('Subtotal de la factura');
            $table->float('iva')->comment('Porcentaje del IVA');
            $table->float('total', 12, 4)->comment('Total de la factura');
